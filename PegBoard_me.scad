...
module pinboard_clips(total_x, height) 
{
	rotate([0,90,0])
	for(i=[0:round(total_x/hole_spacing)]) {
		for(j=[0:max(strength_factor, round(height/hole_spacing))]) {

			translate([
				j*hole_spacing, 
				-hole_spacing*(round(total_x/hole_spacing)/2) + i*hole_spacing, 
				0])
					pin(j==0);
		}
	}
...
module pegstr() 
{
	difference() {
		union() {
			...
			color([1,0,0])
				pinboard_clips(holder_total_x, holder_height);
		}
...
extra_x_separation=2;
extra_y_separation=2;

x_size = (hex_d+wall_thickness+extra_x_separation) * (holder_x_count-1) + wall_thickness*2 + hex_d;
y_size = (hex_d+wall_thickness+extra_y_separation) * (holder_y_count-1) + wall_thickness*2 + hex_d + 2;

extra_z=holder_height/2;

right((board_thickness*1.5+epsilon)/2) down(7)
    pinboard_clips(x_size, holder_height/2);

difference() {
    union() {
        cuboid([y_size,x_size,holder_height/2], align=V_LEFT+V_DOWN, fillet=1, edges=EDGES_ALL-EDGES_Z_RT-EDGES_Y_RT);
        down(holder_height/2-2)
        hull() {
            cuboid([y_size/2,x_size,2],align=V_LEFT+V_DOWN, fillet=1, edges=EDGES_ALL-EDGES_Z_RT-EDGES_Y_RT);
            down(extra_z)
            cuboid([2,x_size,2],align=V_LEFT+V_DOWN, fillet=1, edges=EDGES_ALL-EDGES_Z_RT-EDGES_Y_RT);
        }

    }
    // yspread(hex_d+wall_thickness+extra_x_separation, 5)
    #grid2d(spacing=[hex_d+wall_thickness+extra_x_separation, hex_d+wall_thickness+extra_y_separation], rows=holder_x_count, cols=holder_y_count, align=V_LEFT)
    left(4) up(.01) yrot(0)
        cyl(d=hex_d, h=8, align=V_LEFT+V_DOWN);
