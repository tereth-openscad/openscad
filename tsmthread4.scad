...
module print_npt() {
    ...
    imperial() {
        ...
        difference() {
            union() {
                translate([0,0,15/64-1/8-3/64]) 
                thread_npt(DMAJ=1.05+$OD_COMP, PITCH=0.07143, L=0.07143*8);
                cylinder(d=flat()*1.15, h=5/64, $fn=6);
            }
            
            translate([0,0,5/64+1/8]) cylinder(d=3/4, h=2);
        }
